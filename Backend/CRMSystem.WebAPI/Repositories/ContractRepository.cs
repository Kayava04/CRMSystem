...
namespace CRMSystem.WebAPI.Repositories
{
        ...
        : IBasicRepository<Contract>
    {
        ...
        {
            var entity = await context.Contracts.AsNoTracking().FirstOrDefaultAsync(c => c.Id == id);
            return mapper.Map<Contract>(entity);
        }

        public async Task<IEnumerable<Contract>> GetAllAsync()
        {
            var entities = await context.Contracts.AsNoTracking().ToListAsync();
        ...
        {
            var existing = await context.Contracts.FirstOrDefaultAsync(c => c.Id == id);
            if (existing == null) return null;

            context.Entry(existing).CurrentValues.SetValues(mapper.Map<ContractEntity>(contract));
            await context.SaveChangesAsync();
        ...
        {
            var entity = await context.Contracts.FirstOrDefaultAsync(c => c.Id == id);
            if (entity != null)
            {
